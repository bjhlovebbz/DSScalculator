############################################################
# DSSCalculator - Complete Dose-Response & DSS Computation Pipeline
#
# Description:
# This unified script processes raw high-throughput screening data
# to compute the Drug Sensitivity Score (DSS), AUC, and IC50.
# The pipeline is divided into two main parts:
#   Part 1: Data Preprocessing & Normalization (Control-based)
#   Part 2: Non-linear Curve Fitting & DSS Calculation
#
# Key Features:
# - Robust per-plate normalization using control wells.
# - Adaptive non-linear least-squares (nlsLM) curve fitting for 
#   dose-response data (4-parameter logistic model).
# - Outlier handling and automated curve correction.
# - High-quality dose-response curve visualization (PDF generation).
#
# Input (Expected in 'data/' directory):
#   1. met_plate.xlsx (Raw data)
#   2. trend_example.xlsx (Trend direction matrix)
#
# Output (Saved to 'data/' directory):
#   - Intermediate formatted matrices.
#   - DSS_auc_ic50_all_original.xlsx (Raw calculation results)
#   - DSS_auc_ic50_all_reserved.xlsx (Final results with trend applied)
#   - dose_response_curves_perplate.pdf (Visualizations)
#
# Author: Xuechun Gao
# Year: 2026
############################################################
...
# ==========================================================
# Part 1: Data Preprocessing & Normalization
# ==========================================================
# Description:
# This section preprocesses raw plate reader data, normalizes
# the signals against 'Activated' and 'Not Activated' controls 
# per plate, and prepares the format for downstream fitting.
# ==========================================================
...
# -----------------------
# Helpers: Robust repo root (No setwd needed)
# -----------------------
...
# Automatically resolve paths relative to the script location
...
# -----------------------
# User-Configurable Input Paths
# -----------------------
raw_data_path <- file.path(data_dir, "met_plate.xlsx")
...
if (!file.exists(trend_path))    stop("Trend data file not found: ", trend_path)

df <- read_excel(raw_data_path, sheet = "Background Subtracted")
trend_df <- read_excel(trend_path)

# -----------------------
# 2. Per-Plate Normalization
# -----------------------
# Define phenotypic feature columns
feature_cols <- c(
  "CTG", "Caspase", "RT", "NAD", "NADP(H)",
  "IFNgamma", "TNFalpha", "Glucose", "Lactate", "Malate", "Pyruvate"
)

# Calculate median for Activated / Not Activated controls per plate
activated_median <- df %>%
  filter(DRUG_NAME == "Activated") %>%
  group_by(PLATE) %>%
  summarise(
    across(all_of(feature_cols), ~ median(.x, na.rm = TRUE), .names = "ACT_{.col}"),
    .groups = "drop"
  )

not_activated_median <- df %>%
  filter(DRUG_NAME == "Not Activated") %>%
  group_by(PLATE) %>%
  summarise(
    across(all_of(feature_cols), ~ median(.x, na.rm = TRUE), .names = "NACT_{.col}"),
    .groups = "drop"
  )

# Merge control medians back to the data and calculate normalized values
normalized_df <- df %>%
  filter(!DRUG_NAME %in% c("Activated", "Not Activated")) %>%
  left_join(activated_median, by = "PLATE") %>%
  left_join(not_activated_median, by = "PLATE")

for (col in feature_cols) {
  act_col  <- paste0("ACT_", col)
  nact_col <- paste0("NACT_", col)
  norm_col <- paste0("normalized_", col)
  
  # Percent Inhibition formula
  normalized_df[[norm_col]] <- 100 - (
    (normalized_df[[col]] - normalized_df[[nact_col]]) /
      (normalized_df[[act_col]] - normalized_df[[nact_col]])
  ) * 100
}

# Clean up column names
corrected_df <- normalized_df %>%
  dplyr::select(DRUG_NAME, CONCENTRATION, PLATE, starts_with("normalized_")) %>%
  rename_with(~ sub("^normalized_", "", .x), starts_with("normalized_"))

write_xlsx(corrected_df, file.path(out_dir, "corrected_example.xlsx"))

# -----------------------
# 3. Apply Trend Direction
# -----------------------
# Expand trend dataframe to match the 5 concentrations per drug
trend_expanded <- trend_df[rep(1:nrow(trend_df), each = 5), ]

# Safety check
if (nrow(trend_expanded) != nrow(corrected_df)) {
  stop("Row count mismatch after trend expansion. Please check data consistency.")
}
if (!all(trend_expanded$DRUG_NAME == corrected_df$DRUG_NAME)) {
  warning("Drug names in trend and corrected data may not align perfectly!")
}

corrected_sub <- corrected_df[, feature_cols]
trend_sub     <- trend_expanded[, feature_cols]

# Multiply normalized values by their corresponding trend (+1 or -1)
result <- corrected_sub * trend_sub

final_normalized_df <- cbind(corrected_df[, 1:3], result)
colnames(final_normalized_df)[4:14] <- paste0("normalized_", colnames(final_normalized_df)[4:14])

write_xlsx(final_normalized_df, file.path(out_dir, "normalized_trend.xlsx"))

# -----------------------
# 4. Format for DSS Calculation (Wide to Long)
# -----------------------
formatted_df <- final_normalized_df %>%
  pivot_longer(
...
cat("Part 1: Preprocessing Complete.\n")

# ==========================================================
# Part 2: Dose-Response Curve Fitting & DSS Calculation
# ==========================================================
# Description:
# This section iterates through each drug-screen combination,
# fits 4-parameter logistic curves (handling outliers and edge 
# cases adaptively), and calculates analytical metrics: 
# DSS (Drug Sensitivity Score), AUC, and absolute IC50. 
# It also generates visual plots for quality control.
# ==========================================================
...
results <- formatted_df %>%
  ...
  lapply(function(data_subset) {
    ...
    inhibition_percent[inhibition_percent < -100] <- -100
    # if all 5 points are exactly 100/-100, set the first one to 99/-99
    if (length(inhibition_percent) == 5 && all(inhibition_percent == 100, na.rm = TRUE)) {
      inhibition_percent[1] <- 99
    }
    if (length(inhibition_percent) == 5 && all(inhibition_percent == -100, na.rm = TRUE)) {
      inhibition_percent[1] <- -99
    }
...
head(dss_data)
write_xlsx(dss_results, file.path(out_dir, "DSS_auc_ic50_all_original.xlsx"))
pdf(file.path(out_dir, "dose_response_curves_perplate.pdf"), width = 8, height = 8)
...
cat("Part 2: DSS Calculation and Export Complete. All files saved to:", out_dir, "\n")
